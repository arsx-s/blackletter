...
export async function executeFormatter(ctx: PipelineContext): Promise<FormattedResult> {
  const lines = ctx.response.split("\n");
  ...
  if (sections.length === 0) {
    const totalContent = ctx.response.trim();
    if (totalContent.length > 0) {
      sections.push({ type: "objective", title: "Objective", content: `Understand ${ctx.normalizedPrompt}`, order: 0 });
      sections.push({ type: "argument", title: "Argument", content: totalContent, order: 1 });
      sections.push({ type: "verdict", title: "Verdict", content: `Review the argument above for the key points of ${ctx.normalizedPrompt}.`, order: 2 });
    }
  }
